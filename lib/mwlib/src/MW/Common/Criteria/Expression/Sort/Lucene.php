...
/**
 * Lucene implementation for sorting objects.
 *
 * @package MW
 * @subpackage Common
 */
class MW_Common_Criteria_Expression_Sort_Lucene
	extends MW_Common_Criteria_Expression_Abstract
	implements MW_Common_Criteria_Expression_Sort_Interface
{
	private static $_operators = array( '+' => SORT_ASC, '-' => SORT_DESC );
	private $_operator = '+';
	...
	public function toString( array $types, array $translations = array(), array $plugins = array() )
	{
		...
		return ', "' . addcslashes( $transname, '"' ) . '",' . $types[$name] . ',' . self::$_operators[$this->_operator];
	}
