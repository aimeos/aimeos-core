...
implements MShop_Service_Provider_Interface
{
	...
	{
		throw new MShop_Service_Exception( sprintf( 'An error occured in a service. Method "%1$s" for provider not available.', 'query' ) );
	}
	...
	protected function _checkConfig( array $config, array $attributes )
	{
		...
		foreach ( $config as $key => $def )
		{
			...
			{
				$errors[$key] = sprintf( 'An error occured in a service. Required attribute "%1$s" in provider configuration not available.', $key );
				continue;
			}
			...
			if( isset( $attributes[$key] ) )
			{
				switch( $def['type'] )
				{
					...
					case 'datetime':
						...
						break;
					default:
						throw new MShop_Service_Exception( sprintf( 'An error occured in a service. Invalid characters in attribute for provider configuration. Attribute is not of type "%1$s".', $def['type'] ) );
				}
			}
