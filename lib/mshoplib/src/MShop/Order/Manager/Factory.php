	...
	implements MShop_Common_Factory_Iface
{
	...
	public static function createManager( MShop_Context_Item_Iface $context, $name = null )
	{
		 ...
		 *
		 *  MShop_Order_Manager_Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  MShop_Order_Manager_Mymanager
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/order/manager/name = Mymanager
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/order/manager/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'MShop_Order_Manager_' . $name : '<not a string>';
			throw new MShop_Order_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'MShop_Order_Manager_Iface';
