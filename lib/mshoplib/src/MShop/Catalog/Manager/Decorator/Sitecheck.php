	...
	public function insertItem( MShop_Catalog_Item_Interface $item, $parentId = null, $refId = null )
	{
		if ( $parentId !== null ) {
			...
			if ( $parent->getSiteId() != $siteId ) {
				throw new MShop_Exception( sprintf( 'An error occured in the catalog. Site can not be inserted. Site ID of site differs from site ID of parent site.' ) );
			}
		}
	...
	public function moveItem( $id, $oldParentId, $newParentId, $refId = null )
	{
		...
		foreach ( $manager->searchItems( $search ) as $item )
		{
			if ( $item->getSiteId() != $siteId  ) {
				throw new MShop_Exception( sprintf( 'An error occured in the catalog. Site can not be moved. Site ID of site differs from present site id, site ID of existing parent site "$oldParentId" or site id of new parent site "$newParentId".' ) );
			}
		}
