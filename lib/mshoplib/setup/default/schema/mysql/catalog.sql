...
CREATE TABLE "mshop_catalog_site" (
	-- Unique id
	"id" INTEGER NOT NULL AUTO_INCREMENT,
	-- parent id references mshop_catalog.id
	"parentid" INTEGER NOT NULL,
	-- site id, references mshop_locale_site.id
	"siteid" INTEGER NOT NULL,
	-- value
	...
	-- Editor who modified this entry at last
	"editor" VARCHAR(255) NOT NULL,
CONSTRAINT "pk_mscatsi_id"
	PRIMARY KEY ("id"),
CONSTRAINT "unq_mscatsi_sid_pid"
	UNIQUE ("siteid", "parentid"),
CONSTRAINT "fk_mscatsi_parentid"
	FOREIGN KEY ("parentid")
	REFERENCES "mshop_catalog" ("id")
	ON UPDATE CASCADE
	ON DELETE CASCADE,
CONSTRAINT "fk_mscatsi_siteid"
	FOREIGN KEY ("siteid")
	REFERENCES "mshop_locale_site" ("id")
	ON UPDATE CASCADE
	ON DELETE CASCADE
