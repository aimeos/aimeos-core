 ...
 * @subpackage Jobs
 */
class Controller_Jobs_Order_Cleanup_Unpaid_Factory
	...
	implements Controller_Jobs_Common_Factory_Interface
{
	...
	public static function createController( MShop_Context_Item_Interface $context, Aimeos $aimeos, $name = null )
	{
		 ...
		 *
		 *  Controller_Jobs_Order_Cleanup_Unpaid_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_Jobs_Order_Cleanup_Unpaid_Myunpaid
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/controller/jobs/order/cleanup/unpaid/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Controller_Jobs_Order_Cleanup_Unpaid_' . $name : '<not a string>';
		 ...
		 * ("Controller_Jobs_Order_Cleanup_Unpaid_Decorator_*") around this job controller.
		 *
		 *  controller/jobs/order/cleanup/unpaid/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_Jobs_Order_Cleanup_Unpaid_Decorator_Decorator2" only to this job
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2015.09
