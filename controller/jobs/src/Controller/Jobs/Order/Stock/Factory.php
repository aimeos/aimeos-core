 ...
 * @subpackage Jobs
 */
class Controller_Jobs_Order_Stock_Factory
	...
	implements Controller_Jobs_Common_Factory_Interface
{
	...
	{
		if ( $name === null ) {
			$name = $context->getConfig()->get('classes/controller/jobs/order/stock/name', 'Default');
		}

		if ( ctype_alnum($name) === false )
		{
			$classname = is_string($name) ? 'Controller_Jobs_Order_Stock_' . $name : '<not a string>';
			throw new Controller_Jobs_Exception( sprintf( 'Invalid class name "%1$s"', $classname ) );
		}

		$iface = 'Controller_Jobs_Interface';
		$classname = 'Controller_Jobs_Order_Stock_' . $name;

		$controller = self::_createController( $context, $arcavias, $classname, $iface );
		return self::_addControllerDecorators( $context, $arcavias, $controller, 'order/stock' );
	}
}