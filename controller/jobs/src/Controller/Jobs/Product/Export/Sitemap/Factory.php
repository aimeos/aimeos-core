 ...
 * @subpackage Jobs
 */
class Controller_Jobs_Product_Export_Sitemap_Factory
	...
	implements Controller_Jobs_Common_Factory_Interface
{
	...
	public static function createController( MShop_Context_Item_Interface $context, Arcavias $arcavias, $name = null )
	{
		 ...
		 *
		 *  Controller_Jobs_Product_Export_Sitemap_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_Jobs_Product_Export_Sitemap_Mysitemap
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/controller/jobs/product/export/sitemap/name = Mysitemap
		 *
		 ...
		 */
		if ( $name === null ) {
			$name = $context->getConfig()->get('classes/controller/jobs/product/export/sitemap/name', 'Default');
		}

		if ( ctype_alnum($name) === false )
		{
			$classname = is_string($name) ? 'Controller_Jobs_Product_Export_Sitemap_' . $name : '<not a string>';
		...
		/** controller/jobs/product/export/sitemap/excludes
		 ...
		 * ("Controller_Jobs_Product_Export_Sitemap_Decorator_*") around the job
		 * controller.
		 *
		 *  controller/jobs/product/export/sitemap/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_Jobs_Product_Export_Sitemap_Decorator_Decorator2"
