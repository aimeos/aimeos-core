...
abstract class Base
	...
	implements \Aimeos\Controller\Jsonapi\Common\Decorator\Iface
{
	private $controller;
	...
	{
		parent::__construct( $context, $templatePaths, $path );
	 ...
	 * @throws \Aimeos\Controller\Jsonapi\Exception If method call failed
	 */
	public function __call( $name, array $param )
	{
		if( ( $result = call_user_func_array( array( $this->controller, $name ), $param ) ) === false ) {
			throw new \Aimeos\Controller\Jsonapi\Exception( sprintf( 'Unable to call method "%1$s"', $name ) );
		}
