...
class Controller_ExtJS_Text_DefaultTest extends MW_Unittest_Testcase
{
	...
	{
		$textManager = MShop_Text_Manager_Factory::createManager( TestHelper::getContext() );
		$listManager = $textManager->getSubManager('list');

		$search = $textManager->createSearch();
		$search->setConditions( $search->compare( '==', 'text.label', 'cafe_long_desc' ) );
		$result = $textManager->searchItems( $search );
		...
		$searchParams = (object) array(
			'site' => 'unittest',
			'condition' => (object) array( '&&' => array( 0 => array( '==' => (object) array( 'text.label' => 'cafe_long_desc' ) ) ) )
		);
		...
		$search->setConditions( $search->compare( '==', 'text.list.parentid', $item->getId() ) );
		$listItems = $listManager->searchItems( $search );

		$search = $listManager->createSearch();
		$search->setConditions( $search->compare( '==', 'text.list.parentid', $saved['items']->{'text.id'} ) );
