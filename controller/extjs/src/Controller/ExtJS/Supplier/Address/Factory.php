 ...
 * @subpackage ExtJS
 */
class Controller_ExtJS_Supplier_Address_Factory
	...
	implements Controller_ExtJS_Common_Factory_Iface
{
	...
	{
		/** classes/controller/extjs/supplier/address/name
		 ...
		 *
		 *  Controller_ExtJS_Supplier_Address_Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_ExtJS_Supplier_Address_Mylist
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/controller/extjs/supplier/address/name = Mylist
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/controller/extjs/supplier/address/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Controller_ExtJS_Supplier_Address_' . $name : '<not a string>';
			throw new Controller_ExtJS_Exception( sprintf( 'Invalid class name "%1$s"', $classname ) );
		}

		$iface = 'Controller_ExtJS_Common_Iface';
		$classname = 'Controller_ExtJS_Supplier_Address_' . $name;
		 ...
		 * ("Controller_ExtJS_Supplier_Address_Decorator_*") around the ExtJS controller.
		 *
		 *  controller/extjs/supplier/address/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_ExtJS_Supplier_Address_Decorator_Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2015.09
