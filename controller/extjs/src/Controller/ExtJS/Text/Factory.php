 ...
 * @subpackage ExtJS
 */
class Controller_ExtJS_Text_Factory
	...
	implements Controller_ExtJS_Common_Factory_Iface
{
	...
	{
		/** classes/controller/extjs/text/name
		 ...
		 *
		 *  Controller_ExtJS_Text_Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_ExtJS_Text_Mytext
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/controller/extjs/text/name = Mytext
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/controller/extjs/text/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Controller_ExtJS_Text_' . $name : '<not a string>';
		 ...
		 * ("Controller_ExtJS_Text_Decorator_*") around the ExtJS controller.
		 *
		 *  controller/extjs/text/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_ExtJS_Text_Decorator_Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
