	...
	implements Controller_ExtJS_Common_Factory_Interface
{
	public static function createController( MShop_Context_Item_Interface $context, $name = null )
	{
		 ...
		 *
		 *  Controller_ExtJS_Product_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_ExtJS_Product_Myproduct
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/controller/extjs/product/name = Myproduct
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/controller/extjs/product/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Controller_ExtJS_Product_' . $name : '<not a string>';
			throw new Controller_ExtJS_Exception( sprintf( 'Invalid class name "%1$s"', $classname ) );
		}

		$iface = 'Controller_ExtJS_Common_Interface';
		$classname = 'Controller_ExtJS_Product_' . $name;
		 ...
		 * ("Controller_ExtJS_Product_Decorator_*") around the ExtJS controller.
		 *
		 *  controller/extjs/product/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_ExtJS_Product_Decorator_Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2014.03
		 ...
		 * @see controller/extjs/product/decorators/global
		 */
		return self::_addControllerDecorators( $context, $controller, 'product' );
	}
}