 ...
 * @subpackage ExtJS
 */
class Controller_ExtJS_Product_List_Factory
	...
	implements Controller_ExtJS_Common_Factory_Iface
{
	public static function createController( MShop_Context_Item_Iface $context, $name = null )
	{
		 ...
		 *
		 *  Controller_ExtJS_Product_List_Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_ExtJS_Product_List_Mylist
		 *
		...
		{
			$classname = is_string( $name ) ? 'Controller_ExtJS_Product_List_' . $name : '<not a string>';
			throw new Controller_ExtJS_Exception( sprintf( 'Invalid class name "%1$s"', $classname ) );
		}

		$iface = 'Controller_ExtJS_Common_Iface';
		$classname = 'Controller_ExtJS_Product_List_' . $name;
		 ...
		 * ("Controller_ExtJS_Product_List_Decorator_*") around the ExtJS controller.
		 *
		 *  controller/extjs/product/list/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_ExtJS_Product_List_Decorator_Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2015.09
