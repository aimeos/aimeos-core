 ...
 * @subpackage ExtJS
 */
class Controller_ExtJS_Price_Lists_Factory
	...
	implements Controller_ExtJS_Common_Factory_Iface
{
	public static function createController( MShop_Context_Item_Iface $context, $name = null )
	{
		 ...
		 *
		 *  Controller_ExtJS_Price_Lists_Standard
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Controller_ExtJS_Price_Lists_Mylist
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/controller/extjs/price/list/name', 'Standard' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Controller_ExtJS_Price_Lists_' . $name : '<not a string>';
			throw new Controller_ExtJS_Exception( sprintf( 'Invalid class name "%1$s"', $classname ) );
		}

		$iface = 'Controller_ExtJS_Common_Iface';
		$classname = 'Controller_ExtJS_Price_Lists_' . $name;

		$controller = self::createControllerBase( $context, $classname, $iface );

		/** controller/extjs/price/list/decorators/excludes
		 ...
		 *
		 *  controller/extjs/price/list/decorators/excludes = array( 'decorator1' )
		 *
		 ...
		 * @see controller/extjs/price/list/decorators/local
		 */

		/** controller/extjs/price/list/decorators/global
		 ...
		 *
		 *  controller/extjs/price/list/decorators/global = array( 'decorator1' )
		 *
		...
		/** controller/extjs/price/list/decorators/local
		 ...
		 * ("Controller_ExtJS_Price_Lists_Decorator_*") around the ExtJS controller.
		 *
		 *  controller/extjs/price/list/decorators/local = array( 'decorator2' )
		 *
		 * This would add the decorator named "decorator2" defined by
		 * "Controller_ExtJS_Price_Lists_Decorator_Decorator2" only to the ExtJS
		 * controller.
		 *
		 * @param array List of decorator names
		 * @since 2015.09
