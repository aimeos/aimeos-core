 ...
 * @subpackage Html
 */
class Client_Html_Email_Delivery_Factory
	...
	implements Client_Html_Common_Factory_Interface
{
	...
	{
		/** classes/client/html/email/delivery/name
		 ...
		 *
		 *  Client_Html_Email_Delivery_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Client_Html_Email_Delivery_Mydelivery
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/client/html/email/delivery/name = Mydelivery
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/client/html/email/delivery/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Client_Html_Email_Delivery_' . $name : '<not a string>';
			throw new Client_Html_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'Client_Html_Interface';
		$classname = 'Client_Html_Email_Delivery_' . $name;
		 ...
		 * @see client/html/email/delivery/decorators/global
		 */
		return self::_addClientDecorators( $context, $client, $templatePaths, 'email/delivery' );
	}
