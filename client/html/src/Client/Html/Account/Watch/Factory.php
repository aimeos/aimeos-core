 ...
 * @subpackage Html
 */
class Client_Html_Account_Watch_Factory
	...
	implements Client_Html_Common_Factory_Interface
{
	...
	{
		/** classes/client/html/account/watch/name
		 ...
		 *
		 *  Client_Html_Account_Watch_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Client_Html_Account_Watch_Mywatch
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/client/html/account/watch/name = Mywatch
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/client/html/account/watch/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Client_Html_Account_Watch_' . $name : '<not a string>';
			throw new Client_Html_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'Client_Html_Interface';
		$classname = 'Client_Html_Account_Watch_' . $name;

		$client = self::_createClient( $context, $classname, $iface, $templatePaths );

		return self::_addClientDecorators( $context, $client, $templatePaths, 'account/watch' );
	}
