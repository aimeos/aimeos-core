 ...
 * @subpackage Html
 */
class Client_Html_Catalog_Stock_Factory
	...
	implements Client_Html_Common_Factory_Interface
{
	 ...
	 * @param array Stock of file system paths where the templates are stored
	...
	{
		/** classes/client/html/catalog/stock/name
		 ...
		 *
		 *  Client_Html_Catalog_Stock_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Client_Html_Catalog_Stock_Mystock
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/client/html/catalog/stock/name = Mystock
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/client/html/catalog/stock/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Client_Html_Catalog_Stock_' . $name : '<not a string>';
			throw new Client_Html_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'Client_Html_Interface';
		$classname = 'Client_Html_Catalog_Stock_' . $name;

		$client = self::_createClient( $context, $classname, $iface, $templatePaths );

		return self::_addClientDecorators( $context, $client, 'catalog/stock' );
	}
}
