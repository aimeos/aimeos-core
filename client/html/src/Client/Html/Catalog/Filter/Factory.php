 ...
 * @subpackage Html
 */
class Client_Html_Catalog_Filter_Factory
	...
	implements Client_Html_Common_Factory_Interface
{
	...
	public static function createClient( MShop_Context_Item_Interface $context, array $templatePaths, $name = null )
	{
		 ...
		 *
		 *  Client_Html_Catalog_Filter_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Client_Html_Catalog_Filter_Myfilter
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/client/html/catalog/filter/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Client_Html_Catalog_Filter_' . $name : '<not a string>';
			throw new Client_Html_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'Client_Html_Interface';
		$classname = 'Client_Html_Catalog_Filter_' . $name;
		 ...
		 * @see client/html/catalog/filter/decorators/global
		 */
		return self::_addClientDecorators( $context, $client, 'catalog/filter' );
	}
