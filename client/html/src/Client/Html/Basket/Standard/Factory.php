 ...
 * @subpackage Html
 */
class Client_Html_Basket_Standard_Factory
	...
	implements Client_Html_Common_Factory_Interface
{
	...
	{
		/** classes/client/html/basket/standard/name
		 ...
		 *
		 *  Client_Html_Basket_Standard_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Client_Html_Basket_Standard_Mybasket
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/client/html/basket/standard/name = Mybasket
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/client/html/basket/standard/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Client_Html_Basket_Standard_' . $name : '<not a string>';
			throw new Client_Html_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'Client_Html_Interface';
		$classname = 'Client_Html_Basket_Standard_' . $name;
		 ...
		 * @see client/html/basket/standard/decorators/global
		 */
		return self::_addClientDecorators( $context, $client, $templatePaths, 'basket/standard' );
	}
