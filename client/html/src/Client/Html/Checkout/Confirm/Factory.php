 ...
 * @subpackage Html
 */
class Client_Html_Checkout_Confirm_Factory
	...
	implements Client_Html_Common_Factory_Interface
{
	...
	{
		/** classes/client/html/checkout/confirm/name
		 ...
		 *
		 *  Client_Html_Checkout_Confirm_Default
		 *
		 * and you want to replace it with your own version named
		 *
		 *  Client_Html_Checkout_Confirm_Myconfirm
		 *
		 * then you have to set the this configuration option:
		 *
		 *  classes/client/html/checkout/confirm/name = Myconfirm
		 *
		 ...
		 */
		if( $name === null ) {
			$name = $context->getConfig()->get( 'classes/client/html/checkout/confirm/name', 'Default' );
		}

		if( ctype_alnum( $name ) === false )
		{
			$classname = is_string( $name ) ? 'Client_Html_Checkout_Confirm_' . $name : '<not a string>';
			throw new Client_Html_Exception( sprintf( 'Invalid characters in class name "%1$s"', $classname ) );
		}

		$iface = 'Client_Html_Interface';
		$classname = 'Client_Html_Checkout_Confirm_' . $name;
		 ...
		 * @see client/html/checkout/confirm/decorators/global
		 */
		return self::_addClientDecorators( $context, $client, 'checkout/confirm' );
	}
