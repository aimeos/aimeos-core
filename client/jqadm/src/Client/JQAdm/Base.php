 ...
 */
abstract class Base
	implements \Aimeos\Client\JQAdm\Iface
{
	private $view;
	private $context;
	private $subclients;
	...
	{
		if( !isset( $this->view ) ) {
			throw new \Aimeos\Client\JQAdm\Exception( sprintf( 'No view available' ) );
		}
	 ...
	 */
	protected function addDecorators( \Aimeos\Client\JQAdm\Iface $client, array $templatePaths,
	 ...
	 * @return \Aimeos\Client\JQAdm\Iface Client object
	 */
	protected function addClientDecorators( \Aimeos\Client\JQAdm\Iface $client, array $templatePaths, $path )
	{
	 ...
	 * @return \Aimeos\Client\JQAdm\Iface Sub-part object
	 */
	protected function createSubClient( $path, $name )
	{
		...
		if( $name === null ) {
			$name = $this->context->getConfig()->get( 'client/jqadm/' . $path . '/name', 'Standard' );
		}

		if( empty( $name ) || ctype_alnum( $name ) === false ) {
			throw new \Aimeos\Client\JQAdm\Exception( sprintf( 'Invalid characters in client name "%1$s"', $name ) );
		}

		$subnames = str_replace( ' ', '\\', ucwords( str_replace( '/', ' ', $path ) ) );

		$classname = '\\Aimeos\\Client\\JQAdm\\' . $subnames . '\\' . $name;
		$interface = '\\Aimeos\\Client\\JQAdm\\Iface';
	 ...
	 *
	 * @return array List of sub-clients implementing \Aimeos\Client\JQAdm\Iface ordered in the same way as the names
	 */
