	...
	implements \Aimeos\MShop\Common\Factory\Iface
{
	...
	public static function create( \Aimeos\MShop\ContextIface $context, string $name = null ) : \Aimeos\MShop\Common\Manager\Iface
	{
		 ...
		 */
		if( $name === null ) {
			$name = $context->config()->get( 'mshop/order/manager/name', 'Standard' );
		}

		$iface = \Aimeos\MShop\Order\Manager\Iface::class;
		$classname = '\Aimeos\MShop\Order\Manager\\' . $name;

		if( ctype_alnum( $name ) === false )
		{
			$msg = $context->translate( 'mshop', 'Invalid characters in class name "%1$s"' );
			throw new \Aimeos\MShop\Order\Exception( sprintf( $msg, $classname ) );
		}
